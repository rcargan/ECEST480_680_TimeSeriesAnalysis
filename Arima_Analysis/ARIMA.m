...
%% Load in Data
Ast=readtable('DonorAStool_NormalizedAbundances2.csv'); %check to make sure 1001 columns present
allseqs=cell2mat((table2cell(Ast(1:end,4:1000))));  %convert all data to double
...
distmat= pdist(normalized(1:400,:)); %take distances of each sequence
...
Mdl= arima('Constant',1,'D',0,'Seasonality',21,...
'MALags',3,'SMALags',21, 'ARLags',3);
EstMdl = estimate(Mdl,seq1(1:70)');
%[Y,E] = simulate(EstMdl,314);
[yf, yMSE]= forecast(EstMdl,51,'Y0',seq1(1:70)');
figure
plot(seq1, 'Color',[0.75, 0.75, 0.75],'LineWidth',2);
hold on
h1=plot(70:69+51,yf,'r','LineWidth',2);
%h1= plot(250:249+65,yf, 'r','LineWidth',2); 
hold on
plot([70 70], [0 60000], '--k', 'LineWidth', 2)
plot([120 120], [0 60000], '--k', 'LineWidth', 2)
xlim([0 315])
legend('Original','Predicted')
title('ARIMA Prediction of Bacteriodales Cluster','FontSize',13);
